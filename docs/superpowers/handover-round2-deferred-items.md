...
> **✅ RESOLVED — 2026-07-06** (both items done on `feature/new-layout`; branch NOT merged — user controls integration). Full suite green: **1315 passed / 23 skip / 0 fail**. Round 2 = 100% done. The section below is the original handover, kept for history.
> - **Item 1 → user chose the FIX.** The investigation resolved the decision tree: the live search backend KEEPS `_` (`search.py` `_WORD_RE = re.compile(r"\w+", re.UNICODE)`), so the live highlighter splitting on `_` was inconsistent with its *own* search (options B and C converge). Changed `_HIGHLIGHT_WORD_RE` `[^\W_]+` → `\w+` (now identical to the backend + static `tokenize()`), so `user_role` marks as ONE run and live↔static agree for identifier queries. Entity-safety unchanged (match-on-raw); docstrings honestly re-scoped — a residual stopword + CJK tokenizer difference remains and is documented. Commit **`47c63d9`**.
> - **Item 2 → user chose to REUSE the existing `--okf-error`, NOT add `--okf-danger`.** The codebase already defines a themed `--okf-error` (light `oklch(0.50 0.17 27)` / dark `oklch(0.68 0.17 27)`) that drives the offline-connection dot in the SAME status bar, error toasts, and diff-deletion text — it is already in the parity test and lands ≥AA (5.8:1 light / 6.2:1 dark) where the old fixed `#d64545` was sub-AA on light. Fix = one line (`studio.css:118` `var(--okf-danger,#d64545)` → `var(--okf-error,#d64545)`) + a chip-token lock test; no new token, no parity-test change. Commit **`f693fe3`**. ⇒ **the "add `--okf-danger` to 5 blocks + parity test" plan in ITEM 2 below is SUPERSEDED.**
